...
describe('transformAgentFrontmatter', () => {
  it('transforms tools from comma-separated to YAML record', () => {
    const input = `---
name: test-agent
...
Body content`;
    ...
    const result = transformAgentFrontmatter(input);

    expect(result).toContain('tools:\n  Read: true\n  Write: true\n  Bash: true');
    expect(result).not.toContain('tools: Read, Write, Bash');
  });

  it('transforms color name to hex format', () => {
    const input = `---
name: test-agent
tools: Read
color: cyan
...
Body content`;
    ...
    expect(result).toContain(`color: "${COLOR_NAME_TO_HEX.cyan}"`);
    expect(result).not.toContain('color: cyan');
  });

  it('transforms all supported color names', () => {
...
---`;
  ...
  it('handles tools with mcp prefixes', () => {
    const input = `---
name: test-agent
...
Body`;
  ...
  it('does not transform content without frontmatter', () => {
    const input = 'Just plain content without frontmatter';
    const result = transformAgentFrontmatter(input);
    expect(result).toBe(input);
  });

  it('does not transform frontmatter without closing ---', () => {
    const input = `---
name: test
tools: Read, Write
color: cyan

Body content without closing frontmatter`;
...
Body`;
    ...
    expect(result).toContain('description: A test agent for testing');
  });

  it('leaves unknown color names unchanged', () => {
    const input = `---
name: test
color: unknowncolor
---`;
    ...
    const result = transformAgentFrontmatter(input);
    expect(result).toContain('color: unknowncolor');
  });
...
describe('PLATFORMS', () => {
  ...
  describe('opencode', () => {
    ...
    it('transforms agent frontmatter tools to YAML record', () => {
      const input = `---
name: gsd-executor
tools: Read, Write, Bash
color: yellow
...
<role>Agent body</role>`;
      const output = platform.transformContent(input, '~/.config/opencode/');
      expect(output).toContain('tools:\n  Read: true\n  Write: true\n  Bash: true');
      expect(output).not.toContain('tools: Read, Write, Bash');
    });

    it('transforms agent frontmatter color names to hex', () => {
      const input = `---
name: gsd-executor
tools: Read
color: yellow
...
Body`;
      const output = platform.transformContent(input, '~/.config/opencode/');
      expect(output).toContain('color: "#EAB308"');
...
describe('createInstallPlan', () => {
  ...
  it('includes hooks for claude-code when they exist', () => {
    const plan = createInstallPlan(PLATFORMS['claude-code'], paths, {
      srcDir: '/src',
      existsSync: (p) => p === '/src/hooks/dist',
      version: '1.0.0',
    });

    const hooksTask = plan.find(t => t.description === 'hooks (bundled)');
  ...
  describe('install', () => {
    beforeEach(() => {
      ...
      mockFs.existsSync.mockImplementation((path) => {
        ...
        if (path.includes('hooks/dist')) return true;
